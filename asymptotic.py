...
class asymptotic():
    ...
    def asymptotic_model(self, theta_asy):
        ...
        *args : array-like
        ...
        nu0s = self._asymptotic_relation(numax, dnu, eps, alpha)

        Hs = self._P_envelope(nu0s, hmax, numax, env_width)

        mod = jnp.zeros(len(self._f))

        for n in range(len(nu0s)):
            
            mod += self._pair(nu0s[n], Hs[n], mode_width, d02)
